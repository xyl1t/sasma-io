...
function btnJoinClick(e) {
  if (world.hasLoaded) {
    if (!$("#btnJoin").attr("disabled")) {
      ...
      let w = world.windowWidth / world.renderScaleWidth;
      let h = world.windowHeight / world.renderScaleHeight;
      ...
      world.socket.emit("join", w, h, world.isMobile);
    }
  }
}
...
function keydown(e) {
  ...
  if (
    ...
    // components
    if (world.keyboard["o"]) {
      world.debug.showComponents = !world.debug.showComponents;
    }
  }
}
