...
function addPlayerComponents(playerId, width, height, isMobile) {
  ...
  removeComponent(world, Follow, playerId);
  ...
  Position.x[playerId] = randomNumber(-100, 100); // TODO: FIXME
  Position.y[playerId] = randomNumber(-100, 100);
...
for (let i = 0; i < 20; i++) {
  ...
  for (let i = 0; i < randomNumber(0, 3); i++) {
    const leafId = addEntity(world);
    addComponent(world, Position, leafId);
    Position.x[leafId] = Position.x[treeId] + randomNumber(-60, 60);
    Position.y[leafId] = Position.y[treeId] + randomNumber(-60, 60);
    addComponent(world, Sprite, leafId);
    Sprite.texture[leafId] = world.assetIdMap.tree_green_twigs;
    addComponent(world, Layer, leafId);
    Layer.layer[leafId] = 1;
  }
  for (let i = 0; i < randomNumber(0, randomNumber(5, 30)); i++) {
    const leafId = addEntity(world);
    addComponent(world, Position, leafId);
    Position.x[leafId] = Position.x[treeId] + randomNumber(-100, 100);
    Position.y[leafId] = Position.y[treeId] + randomNumber(-100, 100);
    addComponent(world, Sprite, leafId);
    if (Math.random() > 0.6) {
      Sprite.texture[leafId] = world.assetIdMap.tree_green_large;
    } else {
      Sprite.texture[leafId] = world.assetIdMap.tree_green_small;
    }
    addComponent(world, Layer, leafId);
    // Layer.layer[leafId] = 20;
    Layer.layer[leafId] = leafId*4;
  }
}
...
for (let i = 0; i < 50; i++) {
  ...
  const type = Math.round(randomNumber(0, 4));
  if (type == 0) {
