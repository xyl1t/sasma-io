...
for (let i = 0; i < 15; i++) {
  ...
  addComponent(world, Rotation, playerId);
  ...
  Rotation.angle[playerId] = Math.random() * Math.PI*2;
  Sprite.texture[playerId] = Math.round(Math.random()*10);
}
...
io.on("connection", (socket) => {
  ...
  socket.on("join", () => {
    ...
    Position.x[playerId] = 0
    Position.y[playerId] = 0
    Velocity.x[playerId] = Math.random() * 100 - 50;
    Velocity.y[playerId] = Math.random() * 100 - 50;
    Rotation.angle[playerId] = Math.PI;
