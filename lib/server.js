...
function checkIfEntityVisible(id, meId){
  let entityIsVisible = true;
  if (Position.x[id] > Position.x[meId] + Player.viewDistanceWidth[meId] * 0.6){
    entityIsVisible = false;
  } else if (Position.y[id] > Position.y[meId] + Player.viewDistanceHeight[meId] * 0.6){
    entityIsVisible = false;
  } else if (Position.x[id] < Position.x[meId] - Player.viewDistanceWidth[meId] * 0.6){
    entityIsVisible = false;
  } else if (Position.y[id] < Position.y[meId] - Player.viewDistanceHeight[meId] * 0.6){
    entityIsVisible = false;
  }
