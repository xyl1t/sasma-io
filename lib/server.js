...
function loadAssets() {
  ...
  fs.readdirSync(assetFolder).forEach((file) => {
    ...
    assetIdMap[filename] = id++;
    assetPathMap[filename] = "/assets/" + file;
...
for (let i = 0; i < 15; i++) {
  ...
  Position.x[playerId] = Math.random() * 800;
  Position.y[playerId] = Math.random() * 600;
  Gun.rateOfFire[playerId] = 3000; // ms
...
for (let i = 0; i < 15; i++) {
  ...
  Position.x[treeId] = Math.random() * 800;
  Position.y[treeId] = Math.random() * 600;
  addComponent(world, Sprite, treeId);
  Sprite.texture[treeId] = assetIdMap.treeGreen_large;
}
...
io.on("connection", (socket) => {
  ...
  socket.emit("welcome", assetIdMap, assetPathMap);
  ...
  socket.on("join", () => {
    ...
    Position.x[playerId] = 400;
    Position.y[playerId] = 300;
    Velocity.x[playerId] = 0;
    Velocity.y[playerId] = 0;
    Gun.rateOfFire[playerId] = 3000; // ms
    Body.power[playerId] = 300;
    Body.angle[playerId] = 0;
    Body.rotationSpeed[playerId] = Math.PI;
  });
