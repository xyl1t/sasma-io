...
for (let i = 0; i < 50; i++) {
  ...
  Position.x[treeId] = randomNumber(-Zone.size[zoneEntity2], Zone.size[zoneEntity2]);
  Position.y[treeId] = randomNumber(-Zone.size[zoneEntity2], Zone.size[zoneEntity2]);
...
for (let i = 0; i < 50; i++) {
  ...
  Position.x[boxId] = randomNumber(-Zone.size[zoneEntity2], Zone.size[zoneEntity2]);
  Position.y[boxId] = randomNumber(-Zone.size[zoneEntity2], Zone.size[zoneEntity2]);
...
function serverUpdate() {
  ...
  while (accumulator >= dt) {
    ...
    if (world.currentWaitingTime <= 0 && world.players.length >= world.minPlayers) {
...
function sendWorldToSocket(socket) {
  ...
  socket.emit("serverUpdate", packet, world.players, parseInt(world.currentWaitingTime), world.gameStarted);
}
