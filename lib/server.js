...
import { PickupDropper } from "../common/components/Pimport { Zone } from "../common/components/Zone.js";
...
for (let i = 0; i < 50; i++) {
  ...
  Position.x[treeId] = randomNumber(-Zone.size[zoneEntity], Zone.size[zoneEntity]);
  Position.y[treeId] = randomNumber(-Zone.size[zoneEntity], Zone.size[zoneEntity]);
...
// Add some pickups
for(let i=0;i<5;i++){
  const pickId = addEntity(world);
  addComponent(world, Position, pickId);
  Position.x[pickId] = randomNumber(-300, 300);
  Position.y[pickId] = randomNumber(-300, 300);
  addComponent(world, Sprite, pickId);
  Sprite.texture[pickId] = world.assetIdMap.placeholder;
  addComponent(world, Pickup, pickId);
  Pickup.type[pickId] = randomNumber(0,2);
}
